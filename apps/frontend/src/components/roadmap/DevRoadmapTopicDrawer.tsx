...
import type { RoadmapResource, RoadmapTopic } from "@flowctrl/types";

interface DevRoadmapTopicDrawerProps {
          ...
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-zinc-500">
                {topic.resources.length} available
              </span>
            </div>

            {topic.resources && topic.resources.length > 0 ? (
              <div className="space-y-2.5">
                {topic.resources.map((res: RoadmapResource) => (
                  <a
                    key={res.id}
                    href={res.url}
                    target="_blank"
