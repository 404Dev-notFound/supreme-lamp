  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.detail || "Failed to calculate wellbeing score");
      }
      ...
      setError(
        err?.message ||
          "Failed to connect to Wellbeing Service. Please try again later.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-md">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 rounded-lg bg-white text-black font-semibold hover:bg-zinc-200 transition-colors flex items-center justify-center gap-2 text-sm mt-4 cursor-pointer"
              >
                {loading ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white/5 p-8 rounded-2xl border border-white/10 text-center relative overflow-hidden backdrop-blur-md">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors font-medium text-xs cursor-pointer"
              >
