...
import { UploadCloud, FileText, CheckCircle, AlertTriangle, ShieldAlert, ChevronRight, BarChart } from "lucide-react";
...
export default function ResumeScreener() {
  ...
  const handleUpload = async () => {
    ...
    try {
      // Connect to the microservice running on port 5001
      ...
      <main className="pt-32 px-6 max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-zinc-400 max-w-2xl mx-auto">
            Upload your resume to get an instant ATS compatibility score, identify missing keywords, and receive actionable feedback.
          </p>
        </div>
        ...
        {!result && (
          <div className="max-w-xl mx-auto">
            <div className="glass-card p-10 rounded-2xl border border-white/10 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-zinc-400 mb-8 text-sm">PDF format only. Maximum 8MB.</p>
              ...
              {file && (
                <button 
                  ...
                  disabled={loading}
                  className="mt-6 w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
        ...
        {result && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="relative mb-4">
                  <svg className="w-32 h-32 transform -rotate-90">
                    ...
                    <circle 
                      ...
                      strokeDashoffset={351.86 - (351.86 * result.overall_score) / 100}
                      strokeLinecap="round" 
                    />
                  </svg>
              ...
              <div className="md:col-span-2 flex flex-col gap-6">
                <div className="glass-card p-6 rounded-2xl border border-white/10 flex-1">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold flex items-center gap-2">
                      <ShieldAlert className="w-5 h-5 text-blue-400" /> ATS Compatibility
                    </h3>
                  ...
                  {result.ats_compatibility?.issues?.length > 0 ? (
                    ...
                    <div className="flex items-center gap-3 text-sm text-green-400">
                      ...
                      <span>No major ATS parsing issues found. Formatting looks clean.</span>
                    </div>
                  )}
                </div>
                ...
                <div className="glass-card p-6 rounded-2xl border border-white/10 flex-1">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                    <BarChart className="w-5 h-5 text-primary" /> Top Priority Fixes
                  </h3>
                  <ul className="space-y-3">
                    {result.top_3_priority_fixes?.map((fix: string, i: number) => (
                      <li key={i} className="flex items-start gap-3 text-sm text-zinc-300">
                        <div className="w-5 h-5 rounded-full bg-primary/20 text-primary flex items-center justify-center shrink-0 text-xs font-bold mt-0.5">{i+1}</div>
                        <span>{fix}</span>
                      </li>
                    ))}
                  </ul>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="glass-card p-6 rounded-2xl border border-white/10">
                ...
                <ul className="space-y-4">
                  {result.strengths?.map((str: any, i: number) => (
                    <li key={i} className="bg-white/5 p-4 rounded-xl border border-white/5">
                      <p className="font-medium text-sm text-zinc-200 mb-1">{str.point}</p>
                      <p className="text-xs text-zinc-500">{str.why_it_matters}</p>
                    </li>
                  ))}
                  {(!result.strengths || result.strengths.length === 0) && (
                    <p className="text-sm text-zinc-500">No major strengths identified.</p>
                  )}
                </ul>
              </div>
              ...
              <div className="glass-card p-6 rounded-2xl border border-white/10">
                ...
                <ul className="space-y-4">
                  {result.flaws?.map((flaw: any, i: number) => (
                    <li key={i} className="bg-white/5 p-4 rounded-xl border border-white/5">
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${flaw.severity === 'Critical' ? 'bg-red-500/20 text-red-400' : flaw.severity === 'Major' ? 'bg-orange-500/20 text-orange-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                          {flaw.severity}
                        </span>
                      </div>
                      <p className="font-medium text-sm text-zinc-200 mb-1">{flaw.issue}</p>
                      <p className="text-xs text-zinc-500"><span className="text-primary">Fix:</span> {flaw.fix}</p>
                    </li>
                  ))}
                  {(!result.flaws || result.flaws.length === 0) && (
                    <p className="text-sm text-green-500">No major flaws identified! Great job.</p>
                  )}
                </ul>
              </div>
            </div>
            ...
            {result.section_feedback && (
              <div className="glass-card p-6 rounded-2xl border border-white/10">
                <h3 className="text-lg font-semibold mb-6">Section-by-Section Feedback</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  {Object.entries(result.section_feedback).map(([section, feedback]: [string, any], i: number) => (
                    <div key={i} className="bg-white/5 p-4 rounded-xl">
                      <h4 className="capitalize font-medium text-sm text-zinc-300 mb-2">{section}</h4>
